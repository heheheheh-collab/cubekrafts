...
**Status:** built. Every phase in §16 has landed — 449 tests, typecheck clean, the success condition exercised end to end through the real HTTP surface and behind the real authentication gate. What remains is not code: a domain, an ESP account with SPF and DKIM, a GitHub token, and a Cubekrafts read token. Each turns a piece on; without them Foreman still runs and says plainly what it cannot do. See the README.
