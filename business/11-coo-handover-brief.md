...
- **Multi-language:** searchable globe switcher in the public + dealer headers and dealer Settings. **19 languages, every one fully translated** — English, Hindi, Bengali, Marathi, Gujarati, Tamil, Telugu, Kannada, Malayalam, Punjabi, Urdu, Assamese, Odia, Maithili, Nepali, Konkani, Dogri, Sanskrit, Sindhi. Lazy per-language bundles, per-script Noto fonts loaded only for the active language, RTL for Urdu and Sindhi, `preferred_language` on `dealers` so the choice follows a dealer across devices, unknown/legacy language codes fall back cleanly to English. Trade-glossary constant (`src/i18n/glossary.ts`) keeps "shutter/laminate/ply/sq ft/GST" in English per the code-switching rule.
  - **Santali, Manipuri (Meitei), Bodo and Kashmiri were deliberately removed** (founder call, Aug 2026). AI translation into Ol Chiki and Meetei Mayek could not be produced accurately, and the standing rule is that we do not ship what we cannot stand behind — offering a language that silently serves English is the same failure as a fake number. Add them back only with a real speaker's translation, if a customer ever asks.
...
10. **Get the Hindi and Marathi translations reviewed by a native speaker before the first non-English dealer demo.** All 19 bundles are AI-produced and unreviewed; a clumsy phrase in a dealer's own language costs more trust than English would have. Founder can review Hindi directly; ask a pilot dealer to sanity-check Marathi/Gujarati — that request also flatters them and deepens the relationship.
