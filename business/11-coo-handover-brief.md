...
- **Multi-language:** searchable globe switcher in the public + dealer headers and dealer Settings. 19 of 23 languages fully translated (English, Hindi, Bengali, Marathi, Gujarati, Tamil, Telugu, Kannada, Malayalam, Punjabi, Urdu, Assamese, Odia, Maithili, Nepali, Konkani, Dogri, Sanskrit, Sindhi); **Santali, Manipuri, Bodo and Kashmiri are listed but labelled as English-interface** — no bundles yet, deliberately flagged rather than machine-guessed. Lazy per-language bundles, per-script Noto fonts loaded only for the active language, RTL for Perso-Arabic scripts, `preferred_language` on `dealers` so the choice follows a dealer across devices. Trade-glossary constant (`src/i18n/glossary.ts`) keeps "shutter/laminate/ply/sq ft/GST" in English per the code-switching rule.
...
10. **Get the Hindi and Marathi translations reviewed by a native speaker before the first non-English dealer demo.** The bundles are AI-produced and unreviewed; a clumsy phrase in a dealer's own language costs more trust than English would have. Founder can review Hindi directly; ask a pilot dealer to sanity-check Marathi/Gujarati — that request also flatters them and deepens the relationship. Fill the four missing bundles (Santali, Manipuri, Bodo, Kashmiri) only when a real user asks; until then the picker labels them honestly.
